...
class MailchimpSubscribe_ListController extends BaseController {
    ...
    public function actionSubscribe() {
        ...
        if ($email!='' && $this->_validateEmail($email)) { // validate email
            ...
            require_once(CRAFT_PLUGINS_PATH.'mailchimpsubscribe/vendor/Drewm/MailChimp.php');
            ...
            if ($settings['mcsubApikey']!='' && $settings['mcsubListId']!='') {
                ...
                $api = new \Drewm\MailChimp($settings['mcsubApikey']);
                $result = $api->call('lists/subscribe', array(
                    'id'                => $settings['mcsubListId'],
                    'email'             => array('email'=>$email),
                    'merge_vars'        => $vars
                ));

                if (isset($result['status'])) { // an api error occured
                    ...
                    if (craft()->request->isAjaxRequest())
                    {
                        $this->returnJson(array(
                            ...
                            'message' => 'An API error has occured: ' . $result['code'] . ' - ' . $result['error'],
                            'values' => array(
                                'email' => $email,
                                'vars' => $vars
                            )
                    ...
                    craft()->urlManager->setRouteVariables(array(
                        'mailchimpSubscribe' => array(
                            'success' => false,
                            'errorCode' => $result['code'], // set errorCode to match actual mailchimp api error, see http://apidocs.mailchimp.com/api/1.3/exceptions.field.php
                            'message' => 'An API error occured: ' . $result['code'] . ' - ' . $result['error'],
                            'values' => array(
                                'email' => $email,
                                'vars' => $vars
                            )
